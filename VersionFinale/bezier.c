...
Bezier2 approx_bezier2 (Tableau_Point T, int j1, int j2)
{
    ...
    {
    double alpha = 3.*(double)n/((double)n*(double)n-1);
...
Liste_Bezier2 *simplification_douglas_peucker_b2(Tableau_Point T, int j1, int j2, double d)
{
    ...
    for (j=j1+1; j<=j2 ; j++)
    {
        ...
        if (dmax< dj)
        {
            //printf("Entre les points(%0.f,%0.f) et (%0.f,%0.f)\n",T.tab[j1].x,T.tab[j1].y,T.tab[j2].x,T.tab[j2].y);
            //printf("La distance du Point (%0.f,%0.f) à Bezier  (%0.f,%0.f) (%0.2f,%0.2f) (%0.f,%0.f) vaut %0.2f\n",T.tab[j].x, T.tab[j].y, B.A.x, B.A.y,B.B.x, B.B.y,B.C.x, B.C.y,dj);
            dmax= dj ;
            k= j;
        }
    }
...
void ecrire_bezier2_fichier_EPS(Image I,Ensemble_Bezier2 *EB, FILE *f)
{
    ...
    for (i=0; i<EB->nbr ; i++) /* Parcours des contours */
    {
        ...
        while (cell) /* Tant que cell est non nul */ 
        {
            ...
            fprintf(f,"%0.2f %0.2f %0.2f %0.2f %0.2f %0.2f curveto\n",B3.B.x,H-B3.B.y,B3.C.x,H-B3.C.y, B3.D.x,H-B3.D.y );
            cell = cell->suiv ;
        }
        LB = LB->suiv ;
    }
...
{
    ecrire_bezier2_fichier_EPS(I,EB,f); 
